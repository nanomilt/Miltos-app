...
const enhanceViolations = (violations = {}) => {
  const result = Object.entries(violations).map(([id, data]) => {
    const matchedCyclopt = cycloptViolations.find((_) => _.ruleId === id) || {};
...
router.get('/', async (_req, res) => {
  try {
    ...
    const {
      ...
      .select('_id') // Selecting top-level fields
      .lean();
    ...
    const finalFiles = files.map((file) => {
      ...
      const fileSast = sast.filter((_) => _.path === path);
...
router.get('/pull_requests', async (_, res) => {
